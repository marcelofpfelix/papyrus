# AstroPaper parity decisions

Papyrus should not claim AstroPaper feature parity by default. It should borrow
...
| --- | --- | --- |
| Typed central config resolver | Implemented in Papyrus | Tracked by PP-158. `astro-theme-papyrus/config` loads `paper.config.toml`; the remaining follow-up is consuming-site migration. |
| Pagefind search option | Implemented as demo pattern | `/search/`, Pagefind build output, and browser verification cover the current package demo. `paper.config.toml` now carries the `features.search` flag consumed by layouts. |
...
| Edit/source post links | Implemented | `PaperPostLayout` supports source actions, source copy, source link copy, and Web Share fallback behavior. |
| Share links | Implemented | Post actions use the Web Share API when available and fall back to copying the current URL. |
| Pagination and per-index config | Defer full pagination | Post-card limits and display flags now load from `paper.config.toml`. Full reusable pagination helpers are a separate feature because they need route-shape, page-size, RSS, sitemap, and canonical URL decisions. |
...
| Language, direction, timezone, profile, and verification config | Implemented for serializable config | `paper.config.toml` carries language, direction, timezone, theme/font profile, and Google verification fields. Astro integration wiring remains site-owned because each consuming site chooses its routes and integrations. |
...
Before release notes claim AstroPaper parity, every row above must either be
