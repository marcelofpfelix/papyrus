...
function asProjects(value) {
  ...
  return value.flatMap((item) => {
    ...
    const links = asProjectLinks(record.links);
    const pinned = asBoolean(record.pinned) ?? asNumber(record.pinned);
    return [{
      title,
      description,
...
function asAnalyticsProvider(value) {
  ...
  return provider && ["ga4", "plausible", "umami", "goatcounter", "custom"].includes(provider) ? provider : undefined;
}
...
function asAnalyticsConfig(value) {
  ...
  const includeInDev = asBoolean(record.includeInDev ?? record.include_in_dev);
  return {
...
function asHeadConfig(value) {
  ...
  const scripts = Array.isArray(scriptInput)
    ? scriptInput.flatMap((item) => {
      ...
      if (!src) return [];
      const async = asBoolean(entry.async);
      const defer = asBoolean(entry.defer);
      return [{ src, ...(async !== undefined ? { async } : {}), ...(defer !== undefined ? { defer } : {}) }];
    })
    : [];
...
function readFeatureConfig(record) {
  return Object.fromEntries(
    Object.keys(defaultFeatures).flatMap((key) => {
...
function readHomeConfig(record) {
  const projectLimit = asNumber(record.projectLimit ?? record.project_limit ?? record.project_count ?? record.projects);
  return projectLimit !== undefined ? { projectLimit } : {};
}
...
export function resolvePapyrusConfig(config = {}) {
  return {
    ...
    securityTxt: { ...defaultConfig.securityTxt, ...(config.securityTxt ?? {}) },
    features: { ...defaultFeatures, ...(config.features ?? {}) },
    postCard: { ...defaultPostCard, ...(config.postCard ?? {}) },
  };
}
...
export function githubSourceUrl(site, path, kind = "blob") {
  const repo = asString(site.source?.repo);
  if (!repo) return undefined;
  const branch = asString(site.source?.branch) ?? detectedSourceBranch() ?? "main";
  const normalized = normalizeGithubRepo(repo);
  return `https://github.com/${normalized}/${kind}/${branch}/${path.replace(/^\/+/, "")}`;
}
