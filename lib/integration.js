...
import { getTemplatePublicKeys } from "./template/public-keys.js";
    ...
    return {
        ...
        hooks: {
            "astro:config:setup": async ({ injectRoute, updateConfig }) => {
                ...
                const publicKeys = await getTemplatePublicKeys().catch(() => ({}));
                updateConfig({
                    vite: {
                ...
                for (const { key, pattern, entrypoint, localFiles } of publicKeyRoutes) {
                    if (!publicKeys[key] || localRouteExists(localFiles))
                        continue;
                    injectRoute({ pattern, entrypoint });
                }
            },
