...
- `astro-theme-papyrus/PaperBaseLayout.astro`
- `astro-theme-papyrus/PaperPostLayout.astro`
- `astro-theme-papyrus/PaperHeader.astro`
- `astro-theme-papyrus/PaperFooter.astro`
- `astro-theme-papyrus/PaperPostList.astro`
- `astro-theme-papyrus/PaperCvProfile.astro`
- `astro-theme-papyrus/PaperCvHero.astro`
- `astro-theme-papyrus/PaperCvLinks.astro`
- `astro-theme-papyrus/PaperCvSections.astro`
- `astro-theme-papyrus/PaperCvExportActions.astro`
- `astro-theme-papyrus/PaperCvA4Page.astro`
- `astro-theme-papyrus/PaperJekyllCvPage.astro`
- `astro-theme-papyrus/PaperAiMetadata.astro`
- `astro-theme-papyrus/runtime/PaperBackToTopRuntime.astro`
- `astro-theme-papyrus/runtime/PaperMediaRuntime.astro`
- `astro-theme-papyrus/runtime/PaperPostActionsRuntime.astro`
...
- light block: `:root[data-paper-theme="<name>"]`
- dark block: `:root[data-paper-theme="<name>"].dark`
- color tokens: `--paper-bg`, `--paper-fg`, `--paper-muted`, `--paper-panel`, `--paper-border`, `--paper-accent`, `--paper-code-bg`, `--paper-code-fg`, `--paper-theme-color`
- font tokens: `--paper-font-sans`, `--paper-font-mono`
...
- Header background uses `var(--paper-bg)`.
...
`PaperPostList` supports three views:
...
- Code-copy buttons: generated by Pure's copied Shiki transformer with `navigator.clipboard.writeText(this.dataset.code)` and the `.copied` state. There is no Paper hidden-textarea fallback or prompt-stripping layer.
- Line numbers and language labels: rendered through the copied Pure CSS structure and Paper token colors.
...
`PaperPostLayout` composes small runtime components instead of keeping one large
post script:

- `PaperPostActionsRuntime`: share, copy Markdown, and copy citation buttons.
- `PaperMediaRuntime`: optional artifact hydration, Mermaid rendering, and
  image/SVG zoom.
- `PaperBackToTopRuntime`: scroll-aware back-to-top button.
...
`PaperPostLayout` can emit machine-readable metadata and copy actions when the
...
- JSON-LD `BlogPosting` metadata through `PaperAiMetadata`
...
```astro
---
import { PaperAiMetadata } from "astro-theme-papyrus/components";
---
...
- `PaperGithubCard`: client-side GitHub repo preview card.
- `PaperLinkPreview`: deterministic link preview card without metadata scraping.
- `PaperTimeline`: Pure-style timeline component.
- `PaperProjectList`: static project cards.
...
post features built into `PaperBaseLayout` or `PaperPostLayout`, use a grouped
feature config:
...
`PaperBaseLayout` defaults to the package brand: a Twinkling SVG mark plus the
visible site title. The mark uses `currentColor`, so the header logo switches to
`--paper-accent` on hover/focus through normal link color.
...
```ts
import { defaultPaperFeatures, resolvePaperFeatures } from "astro-theme-papyrus/utils";
```
...
```ts
import { definePaperPlugin, resolvePaperPluginConfig } from "astro-theme-papyrus/utils";
...
```ts
const kbdPlugin = definePaperPlugin({
  name: "paper-kbd",
...
`examples/papyrus-kbd-plugin`. It imports `definePaperPlugin` from the public
...
site opts in with `resolvePaperPluginConfig()`.
...
- Add a feature flag when the behavior is optional inside `PaperBaseLayout` or
  `PaperPostLayout`.
...
- Keep Paper-specific styling, route names, and content assumptions thin enough
...
Comments are optional adapters, not required theme state. The package exports `PaperGiscusComments.astro`, so a site can opt in with its own public repo, Discussions category, and IDs.
...
Post stats use `PaperRemotePostStats.astro`:

```astro
<PaperRemotePostStats endpoint="/post-stats.json" slug="my-post" trackView />
```
...
`PaperCvProfile` accepts a normalized object so adapters can load TOML, JSON, or Astro data collections without coupling the component to one file format.
...
`PaperCvUser` object. The package owns the data shape, section rendering
...
1. Pick a built-in `PaperCvProfile` variant for fast layout changes.
2. Use `sectionOverrides` to rename, hide, reorder visually, or restyle known
   sections without changing the CV data.
3. Compose smaller components such as `PaperCvHero`, `PaperCvLinks`,
   `PaperCvSections`, `PaperCvExportActions`, `PaperCvA4Page`, and
   `PaperJekyllCvPage` in a custom Astro route when the built-in profile is not
   enough.
...
Use `PaperJekyllCvPage` when you want the old jekyllcv-compatible page
...
```astro
---
import { PaperJekyllCvPage } from "astro-theme-papyrus/components";
---

<PaperJekyllCvPage user={user} />
```
...
<section class="my-cv-template">
  <PaperCvHero user={user} kicker="whoami" />
  <PaperCvLinks links={user.links} />
  <PaperCvSections sections={user.sections} />
  <PaperCvExportActions json={json} markdown={markdown} />
</section>
```
...
```astro
---
import { PaperCvA4Page } from "astro-theme-papyrus/components";
import "astro-theme-papyrus/paper.css";
---

<main class="paper-cv-print-shell">
